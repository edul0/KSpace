...
function renderSkeleton() {
    document.getElementById('app-container').innerHTML = `
        <header>
            <div class="header-left">
                ...
                <div id="online-indicator" onclick="togglePanel('user-panel')">
                    ...
                    <span id="user-count" style="font-size:10px; font-weight:bold;">Offline</span>
                </div>
            </div>
            <div style="display:flex; gap:10px; align-items:center;">
                <img src="${user.avatar}" onclick="manageProfile()" style="width:28px; border-radius:50%; border:1px solid #000; cursor:pointer;">
                <button onclick="nukeRoom()" style="background:none; border:none; font-size:18px; cursor:pointer;">☢️</button>
                <button onclick="togglePanel('log-panel')" style="background:none; border:none; font-size:18px; cursor:pointer;">📜</button>
        ...
        <div id="user-panel" class="side-panel">
            <div class="panel-header">USUÁRIOS ONLINE <button onclick="togglePanel('user-panel')" style="background:none; border:none; color:#0f0; cursor:pointer;">✖</button></div>
...
async function initData() {
    ...
    if (!data) {
        state = [{id:"todo", title:"A Fazer", cards:[]},{id:"doing", title:"Em Curso", cards:[]},{id:"done", title:"Feito", cards:[]}];
    ...
    channel
        ...
        .on('presence', { event: 'sync' }, () => {
            ...
            const users = [];
            Object.values(presenceState).forEach(arr => arr.forEach(p => { if(!users.find(u=>u.name===p.name)) users.push(p); }));
...
async function nukeRoom() { if(confirm("☢️ Resetar sala?")) { state.forEach(col => col.cards = []); await save("SALA RESETADA"); renderBoard(); } }
...
function manageProfile() { const m = prompt("1-GitHub, 2-Manual, 3-Reset", "2"); if(m==="1"){ const n=prompt("User:"); if(n){localStorage.clear(); localStorage.setItem('kanban_user',n); location.reload();}} else if(m==="2"){ const n=prompt("Nome:"); const a=prompt("Foto URL:"); if(n&&a){localStorage.setItem('kanban_custom_name',n); localStorage.setItem('kanban_custom_avatar',a); location.reload();}} else if(m==="3"){localStorage.clear(); location.reload();} }
